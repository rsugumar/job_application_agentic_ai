...
form_extractor_agent = LlmAgent(
    ...
    3. Extract the form details and return it in dictionary format.
...
form_filler_agent = LlmAgent(
    ...
    Your primary goal is to open the given url using Playwright MCP Tool to extract form details and fill out the form using the information {rag_response} if available.
    ...
    1. Open the given URL using Playwright MCP Tool.
    2. Parse the form fully and understand the form fields.
    3. if {rag_response} is available, use it to fill out the form.
    4. Don't fill out the form if the information is not available.
